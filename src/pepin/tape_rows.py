...
def costmap_row(msg: Any, now: float) -> dict[str, Any]:
    ...
    2026-09-22 and is in ``odom`` since (ros/params/nav2_params.yaml), so a reader that overlays
    these cells on the room has to be told which, and no tape has to be dated.
    """
    info = msg.info
    return {
        "t": stamp(msg.header, now),
...
def loc_row_from_pose(msg: Any, now: float) -> dict[str, Any]:
    """The same record read from a republished ``map -> base_link`` pose (a PoseStamped on
    ``/pose``, the goal server's).

    ``source`` is ``tf`` and the fields are the transform's: this IS that edge, one hop later,
    and a replay must not have to know which node deserialised it. No ``confidence``, for the
    same reason as :func:`loc_row_from_transform`.
    """
    return {
        "t": stamp(msg.header, now),
        "topic": "loc",
        ...
        "theta": round(yaw(msg.pose.orientation), 5),
    }
