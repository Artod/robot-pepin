...
def test_the_pipeline_reproduces_the_node_s_chain_bit_for_bit() -> None:
    ...
    # the affine law alone on the beams alone, as the node ran then: the floor's, the wall's and
    # the parallax's pairs all ship on today, and each of them is a ruler the node had not
...
def test_the_walk_stops_where_a_table_top_recedes_and_can_correct_without_pairs() -> None:
    ...
    law = AffineLaw()
    law.seed(1.4, 0.0)
    # the seeded law must still be the seed under the correction: the floor's, the wall's and the
    # parallax's pairs (all on by default today) would refit it, and this test is about the walk
    pipeline = standard_pipeline(
        law, wall_anchor=False, wall_correct=True, floor_pairs=False, parallax_anchor=False
    )
...
def test_the_range_law_publishes_through_the_affine_law_until_two_bins_fill() -> None:
    ...
    # the beams alone: one wall is one bin only while they are the whole pool, and the floor's
    # and the wall's pairs (on by default today) span the range on their own
    pipeline = standard_pipeline(
        law, range_stage=stage, floor_pairs=False, wall_anchor=False, parallax_anchor=False
    )
    raw = _network(_scene(2.0), 1.3, 0.0, noise=0.0, seed=0)
    result = pipeline.run(raw, _context(_wall_returns(2.0)))
