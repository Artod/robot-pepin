...
| `depth_fusion` | `imu_lean` | bool | off | yes | a frame is placed with the cart's lean at its stamp (pepin.lean, from /imu/data_raw) composed on base_link before the planar odometry, instead of as if the cart stood level; off, the lean is estimated and reported but not applied |
...
`pepin.frame_pose.FramePoser` behind each node's `imu_lean` flag. Roll is positive right side
down, pitch positive nose down. Every node that estimates it prints it in its report line
(`lean +0.3/-1.8 deg q0.94 bias 0.05 deg/s`) whether the flag is on or off, so it can be watched
