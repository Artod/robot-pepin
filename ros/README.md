...
| `rtabmap_frame` | `graph_measurement` | bool | off | yes | beside a known map, publish RTAB-Map's correction of the tracker's own pose as a measurement on /localization/graph_measurement (source "graph") every time the graph moves, for the board's fusion to weigh like any other word; off, the correction only moves this laptop's map -> rtabmap and nothing reaches the pose |
...
- **`graph_measurement`** — bool, default off
  - *What:* beside a known map, publish RTAB-Map's correction of the tracker's own pose as a measurement on /localization/graph_measurement (source "graph") every time the graph moves, for the board's fusion to weigh like any other word; off, the correction only moves this laptop's map -> rtabmap and nothing reaches the pose
