...
| `depth_stream` | `ray_law` | bool | off | yes | the law's scale follows the ray's angle off the optical axis, a / D + b fitted per elevation (pepin.elevation) instead of one pair of numbers for the whole picture; off, the affine law's image stands. A property of the camera and the network, so the neck may tilt without refitting; off by default until it is measured on the robot, and it falls back to the affine law wherever the pool cannot carry it |
