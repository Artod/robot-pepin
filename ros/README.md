...
| `depth_fusion` | `paint_sigma_m` | number 0.01..2 | 0.1 | yes | how sure of itself the tracker must be, in metres of sigma_xy, before this node paints with its pose — read from /localization/sigma, and ignored entirely while nothing publishes that topic (the fit gate stands on its own until it exists) |
...
| `rtabmap_frame` | `graph_trust` | choice: agreement, distance, flat | agreement | yes | what the graph's word carries as its fit. agreement: how well the last 10 words inside 10 s track the tracker's pose carried forward by odometry between them, exp(-rms residual / 0.10 m), times the recognition predicate (graph_words_need_recognition). distance: 1.0 the moment a loop closure or a proximity link ties the present to an older node, decaying as exp(-d / graph_trust_m) with the metres driven since, as between 2026-09-14 and 2026-09-15. flat: every word claims 1.0, as before 2026-09-14. Never above 0.3 in either measured mode while the anchor came from a file and nothing has been recognised yet (pepin.graphtrust, fed from /rtabmap/info) |
| `rtabmap_frame` | `graph_words_need_recognition` | bool | on | yes | a graph word — measurement or candidate — is published only while RTAB-Map is RECOGNISED: it has accepted a loop closure, a proximity link or a localisation against the database it LOADED since its own start (the matched node id is older than the first node this start built), and that acceptance is no more than recognition_max_s of DRIVING old. Unrecognised, the node stays silent, counts the words it withheld, and does not re-learn the anchor either; off, every word is published as before 2026-09-15 |
| `rtabmap_frame` | `recognition_max_s` | number 1..3600 | 120.0 | yes | how many seconds of DRIVING a recognition stands for before the graph goes quiet again (graph_words_need_recognition); time at a standstill does not count |
...
- **`paint_sigma_m`** — number 0.01..2, default 0.1
...
- **`graph_words_need_recognition`** — bool, default on
  - *What:* a graph word — measurement or candidate — is published only while RTAB-Map is RECOGNISED: it has accepted a loop closure, a proximity link or a localisation against the database it LOADED since its own start (the matched node id is older than the first node this start built), and that acceptance is no more than recognition_max_s of DRIVING old. Unrecognised, the node stays silent, counts the words it withheld, and does not re-learn the anchor either; off, every word is published as before 2026-09-15
  - *Default:* on — on, because an unlinked segment is the one failure a graph cannot report itself. Tape 0333, 2026-09-15: RTAB-Map had restarted many times that day and had recognised nothing since its last start, so its current nodes were placed by this session's odometry alone — the anchor on file (-10.33, +1.41, +53.3 deg) was still right for the LINKED nodes it was measured on, the correction read (-0.44, -1.94, -151.8 deg), and the words were odometry dressed as a pose: 19 of them taken, the last 103 cm from the tracker with 102 refused as too far, and the pose flew. The predicate is the matched id against this start's first node id, which is what tells a tie to the map on disk from a tie inside the new segment; the expiry is driving seconds and not wall seconds because a cart parked at the charger has not moved away from what it recognised
...
- **`recognition_max_s`** — number 1..3600, default 120.0
  - *What:* how many seconds of DRIVING a recognition stands for before the graph goes quiet again (graph_words_need_recognition); time at a standstill does not count (1..3600)
  - *Default:* 120.0 — 120 s, because at the cart's 0.3 m/s that is some 30 m of travel — six decay lengths of graph_trust_m, past which the 0.79 m and 31 deg per 25 m the EKF odometry drifts (2026-09-14) has made the segment a map of its own. Driving seconds rather than wall seconds so that an hour on the charger costs nothing
  - *On when:* raise it where the camera sees a known place only once a lap and the word is being withheld between laps while the lidar agrees with it
  - *Off when:* lower it to make the gate bite sooner in a room where the graph closes loops constantly and a stale recognition is never more than seconds old
