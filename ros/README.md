...
| `goal_server` | `controller` | choice: mppi, rpp | mppi | yes | what follows the plan: mppi is Nav2's MPPI controller for every planner, held to the mark's heading by the yaw-checking goal checker; rpp is each planner's own Regulated Pure Pursuit from PLANNERS, ending on position alone as before 2026-09-23. Published latched on controller_selector and goal_checker_selector, so a change is read by the behaviour tree at its next tick |
...
- **`controller`** — choice: mppi, rpp, default mppi
  ...
  - *On when:* mppi by default, and whenever the heading at the mark matters
