...
FLAGS = FlagSet(
    ...
    Flag(
        ...
        " the lattice a fan is matched on is a plateau, a rival 6 cm away scoring 0.99 of the"
        " winner at every window, so the answer inside the window is a tie-break and the window"
        " is what keeps the tie-break beside the belief the lidar and the odometry hold. The 42"
        " -82 % of matches that come back as bounds (``edge``) are that clipping, and it is the"
        " clipping that keeps the camera at 9 cm: widening to 0.50 m leaves 19-56 % of them"
        " bounds anyway",
        on_when="nothing measured asks for it. A wider window does not pull a poor belief back"
        " — it lets the fan walk away from it, and it costs: a 0.50 m window is 444-694 ms a"
        " match on this Mac against 8-12 ms at 0.09 (the same replay), which no camera cadence"
        " can pay. Wider than 0.09 belongs to somebody holding a measurement that the camera"
        " pins a pose it cannot see from the belief, and to a two-stage search"
        " (:meth:`pepin.localization.Localizer.coarse_measure`, 9-15 ms at 0.50 m)",
    ...
    Flag(
        ...
        why="9 degrees for the same reason the 0.09 m stands, and measured with it on the tapes"
        " of 2026-09-14: the heading error against the lidar truth is -0.4...-4.8 degrees median"
        " in this window and -0.2...-20 degrees when the window is opened to 20, the fan's"
        " heading running away with its position. A fan of +-40 degrees does not pin a heading"
        " any better than it pins a place; the belief it starts from is never more than a degree"
        " or two out while the lidar is alive, and that is what it is there to refine",
        on_when="after a stretch on odometry alone, where the heading is what drifts — and only"
        " with the position window left alone, since the two were swept together and only their"
        " pair was measured",
