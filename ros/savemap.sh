...
# (Online SLAM builds its map on the laptop instead: ros/map.sh save NAME.)
...
if ! ssh "root@$BOARD" "docker logs pepin-ros 2>&1 | grep -q 'slam_toolbox\]: Activating'"; then
    echo "slam_toolbox is not running in the container: nothing to save (ros/mode.sh slam_toolbox first)"; exit 1
fi
